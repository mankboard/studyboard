  ...
  return (
    ...
    pb-10'>
      {activeMenu && (<>
        ...
        text-slate-900'>
          {/* Shoppy */}
            <SiShopware style={{color}}/> <span style={{color}}> MankBoard </span>
          </Link>
          <TooltipComponent content='Menu' position='BottomCenter'>
            <button type='button' className='text-xl rounded-full p-3 
          hover:bg-light-gray mt-4 block md:hidden'
              onClick={() => { setactiveMenu((prevActiveMenu) => {
                !prevActiveMenu
              }) }}
            >
              <MdOutlineCancel style={{color}}/>
            </button>
          </TooltipComponent>
        </div>
        
        <div className='mt-10 '>
          {/* WHY USED WRONG BRACKETS IN BELOW's ARROW FUNC. */}
          {links.map((item) => (
            <div key={item.title}>
              <p className='text-gray-400 m-3 mt-4 uppercase font-semibold' style={{color}}>
                {item.title}
              </p>
              {item.links.map((link) => (
                <NavLink
